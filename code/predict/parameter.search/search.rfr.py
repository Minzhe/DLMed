...
#############################  function  ##############################
def RFR_model(X_train, X_test, y_train, y_test, save_path, return_dict, n_estimators, max_features, criterion, max_depth, random_state=1234, verbose=1):
    '''
    ...
    return_dict[(n_estimators, max_features, criterion, max_depth)] = n_estimators, max_features, criterion, max_depth, r2_train, r2_test
...
if __name__ == '__main__':
    ...
    for n_ in n_estimators:
        for f_ in max_features:
            for c_ in criterion:
                for d_ in max_depth:
                    p = mp.Process(target=RFR_model, args=(X_train, X_test, y_train, y_test, model_path, return_dict, n_, f_, c_, d_,))
                    jobs.append(p)
                    p.start()
    ...
    result = pd.DataFrame(list(return_dict.values()), columns=['n_estimators', 'max_features', 'criterion', 'max_depth', 'r2_train', 'r2_test'])
